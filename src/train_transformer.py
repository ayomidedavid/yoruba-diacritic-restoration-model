...
def train(args):
    ...
    model = Seq2SeqTransformer(len(vocab), sos_index=vocab.sos_index, eos_index=vocab.eos_index).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        if dev_dl is not None and not getattr(args, 'no_eval', False):
            try:
                ...
                if f1 > best_f1:
                    best_f1 = f1
                    ckpt_path = os.path.join(args.output_dir, 'transformer_best.pt')
    ...
    last_path = os.path.join(args.output_dir, 'transformer_last.pt')
